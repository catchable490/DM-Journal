...
class OBRService {
      ...
      if (typeof window !== 'undefined' && window.location !== window.parent.location) {
        OBR.onReady(async () => {
          ...
          OBR.scene.onReadyChange((ready) => {
            if (ready) {
              this.fetchSceneName().then(n => this.onSceneCallbacks.forEach(cb => cb(n)));
            }
          });
        });
      } else {
  ...
  async checkIsGM() {
    ...
    try {
      // Race OBR.player.getRole() with a 600ms timeout to prevent hanging
      const rolePromise = OBR.player.getRole();
      const timeoutPromise = new Promise(res => setTimeout(() => res('GM'), 600));
  ...
  onSceneChange(cb) { this.onSceneCallbacks.push(cb); }
...
class JournalManager {
  ...
  generateMarkdown(session) {
    ...
    if (s.entries.length === 0) return md + '*No notes recorded yet. Press `J` or click Quick Note to add notes!*\n';
    const grouped = {};
...
async function main() {
  ...
  const hub = new JournalHub(app, popup);

  // Non-blocking background GM check
  obrService.checkIsGM().then(isGM => {
    if (!isGM) {
      app.innerHTML = '<div class="gm-guard"><h2>GM Only Tool</h2></div>';
    }
  });
}
